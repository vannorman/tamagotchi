// good lord why
const whatAreModules  {
        ADDRESS_CLAMP_TO_EDGE=pc.ADDRESS_CLAMP_TO_EDGE;
        ADDRESS_MIRRORED_REPEAT=pc.ADDRESS_MIRRORED_REPEAT;
        ADDRESS_REPEAT=pc.ADDRESS_REPEAT;
        BLEND_NORMAL=pc.BLEND_NORMAL;
        CULLFACE_NONE=pc.CULLFACE_NONE;
        FILTER_NEAREST=pc.FILTER_NEAREST;
        FILTER_LINEAR=pc.FILTER_LINEAR;
        FILTER_NEAREST_MIPMAP_NEAREST=pc.FILTER_NEAREST_MIPMAP_NEAREST;
        FILTER_LINEAR_MIPMAP_NEAREST=pc.FILTER_LINEAR_MIPMAP_NEAREST;
        FILTER_NEAREST_MIPMAP_LINEAR=pc.FILTER_NEAREST_MIPMAP_LINEAR;
        FILTER_LINEAR_MIPMAP_LINEAR=pc.FILTER_LINEAR_MIPMAP_LINEAR;
        INDEXFORMAT_UINT8=pc.INDEXFORMAT_UINT8;
        INDEXFORMAT_UINT16=pc.INDEXFORMAT_UINT16;
        INDEXFORMAT_UINT32=pc.INDEXFORMAT_UINT32;
        PROJECTION_ORTHOGRAPHIC=pc.PROJECTION_ORTHOGRAPHIC;
        SEMANTIC_POSITION=pc.SEMANTIC_POSITION;
        SEMANTIC_NORMAL=pc.SEMANTIC_NORMAL;
        SEMANTIC_TANGENT=pc.SEMANTIC_TANGENT;
        SEMANTIC_COLOR=pc.SEMANTIC_COLOR;
        SEMANTIC_BLENDINDICES=pc.SEMANTIC_BLENDINDICES;
        SEMANTIC_BLENDWEIGHT=pc.SEMANTIC_BLENDWEIGHT;
        SEMANTIC_TEXCOORD0=pc.SEMANTIC_TEXCOORD0;
        SEMANTIC_TEXCOORD1=pc.SEMANTIC_TEXCOORD1;
        SEMANTIC_TEXCOORD2=pc.SEMANTIC_TEXCOORD2;
        SEMANTIC_TEXCOORD3=pc.SEMANTIC_TEXCOORD3;
        SEMANTIC_TEXCOORD4=pc.SEMANTIC_TEXCOORD4;
        SEMANTIC_TEXCOORD5=pc.SEMANTIC_TEXCOORD5;
        SEMANTIC_TEXCOORD6=pc.SEMANTIC_TEXCOORD6;
        SEMANTIC_TEXCOORD7=pc.SEMANTIC_TEXCOORD7;
        TYPE_INT8=pc.TYPE_INT8;
        TYPE_UINT8=pc.TYPE_UINT8;
        TYPE_INT16=pc.TYPE_INT16;
        TYPE_UINT16=pc.TYPE_UINT16;
        TYPE_INT32=pc.TYPE_INT32;
        TYPE_UINT32=pc.TYPE_UINT32;
        TYPE_FLOAT32=pc.TYPE_FLOAT32;
        math=pc.math;
        ...
        getIndexComponentType = (indexFormat) => {
            switch (indexFormat) {
        ...
        getAccessorType = (componentCount) => {
...
const notAModuleInstance = new whatAreModules(); // I love that I can be this bad at Javascript

whatAreModules.GltfExporter  =  class extends CoreExporter {
                ...
                const collectMeshInstances = (meshInstances) => {
                    meshInstances.forEach((meshInstance) => {
                        ...
                        if (materials.indexOf(material) < 0) {
                            ...
                            // collect textures
                            notAModuleInstance.textureSemantics.forEach((semantic) => {
            ...
            convertTextures(srcTextures, options) {
                ...
                srcTextures.forEach((srcTexture) => {
                    const promise = new whatAreModules.GltfExporter().textureToCanvas(srcTexture, textureOptions);
                    promise.then((canvas) => {
            ...
            buildJson(resources, options) {
                ...
                const promises = new whatAreModules.GltfExporter().convertTextures(resources.textures, options);
                return Promise.all(promises).then((textureCanvases) => {
                    ...
                    new whatAreModules.GltfExporter().writeBuffers(resources, json);
                    new whatAreModules.GltfExporter().writeBufferViews(resources, json);
                    new whatAreModules.GltfExporter().writeCameras(resources, json);
                    new whatAreModules.GltfExporter().writeNodes(resources, json);
                    new whatAreModules.GltfExporter().writeMaterials(resources, json);
                    new whatAreModules.GltfExporter().writeMeshes(resources, json);
                    new whatAreModules.GltfExporter().writeTextures(resources.textures, textureCanvases, json, options);
            ...
            build(entity, options = {}) {
                ...
                return new whatAreModules.GltfExporter().buildJson(resources, options).then((json) => {
